from kivy.utils import platform
from kivy_garden.mapview import MapMarker
from utils.AndroidGPS import AndroidGPS


class MapHandler:
    def __init__(self, map_view, app=None):
        self.map_view = map_view
        self.app = app
        self.lat = None
        self.lon = None
        self.gps = None
        self.user_marker = None

    def request_location_permission(self):
        """Start GPS with permission request (event-driven callback, no delay)"""
        try:
            print("=== MapHandler: Starting GPS with permission request ===")
            self.gps = AndroidGPS(
                on_location_callback=self.on_location,
                on_status_callback=self._on_gps_status
            )
            self.gps.request_permissions_and_start(min_time=1000, min_distance=0)
        except Exception as e:
            print(f"GPS not available: {e}")
            import traceback
            traceback.print_exc()
            self._on_gps_status("GPS: Error starting")

    def start_gps_without_permission_request(self):
        """Start GPS without requesting permissions (already granted)"""
        try:
            print("=== MapHandler: Starting GPS ===")
            self.gps = AndroidGPS(
                on_location_callback=self.on_location,
                on_status_callback=self._on_gps_status
            )
            self.gps.start_without_requesting_permissions(min_time=1000, min_distance=0)
        except Exception as e:
            print(f"GPS not available: {e}")
            import traceback
            traceback.print_exc()
            self._on_gps_status("GPS: Error starting")

    def _on_gps_status(self, status):
        """Forward GPS status to the app"""
        if self.app:
            self.app.update_gps_status(status)

    def on_location(self, **kwargs):
        """Called when GPS location is updated"""
        self.lat = kwargs.get('lat')
        self.lon = kwargs.get('lon')
        accuracy = kwargs.get('accuracy', 0)
        provider = kwargs.get('provider', 'unknown')

        if self.lat and self.lon:
            self.map_view.center_on(self.lat, self.lon)

            if self.user_marker:
                self.user_marker.lat = self.lat
                self.user_marker.lon = self.lon
            else:
                self.user_marker = MapMarker(lat=self.lat, lon=self.lon)
                self.map_view.add_marker(self.user_marker)

            if self.app:
                self.app.update_gps_status(f"GPS: Ready  |  {accuracy:.0f}m")

    def center_map_on_user_location(self):
        """Center map on current GPS location"""
        if self.lat and self.lon:
            self.map_view.center_on(self.lat, self.lon)
